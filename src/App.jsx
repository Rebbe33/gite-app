...
function AddGiteModal({ onSave, onClose }) {
  ...
  const handleFile = async (e) => {
    ...
    try { setTasks(await parseTasksExcel(f)) }
    catch (err) { setError(err.message); setTasks(null) }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave(name.trim(), tasks || [], {
        mode_suivi: modeSuivi, proprietaire,
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-grid">
          ...
          {modeSuivi==='taux_horaire' && <div className="form-field full"><label>Taux horaire (€/h)</label>
            <input type="number" step="0.5" min="0" value={tauxHoraire} onChange={e=>setTauxHoraire(e.target.value)} placeholder="12.00"/>
          </div>}
          {modeSuivi==='forfait' && <div className="form-field full"><label>Montant forfait (€)</label>
...
function GiteSettings({ gite, onRename, onDelete, onUpdate, onClose }) {
  ...
  const handleSave = () => {
    ...
    onUpdate(gite.id, {
      proprietaire, mode_suivi: modeSuivi,
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="form-grid">
          ...
          {modeSuivi==='forfait' && <div className="form-field full"><label>Montant par passage (€)</label>
            <input type="number" step="1" min="0" value={forfaitMontant} onChange={e=>setForfaitMontant(e.target.value)} placeholder="50.00"/>
          </div>}
        </div>
        <div className="modal-actions">
          <button className="btn-danger" onClick={()=>window.confirm(`Supprimer ${gite.nom} ?`)&&onDelete(gite.id)}>
...
export default function App() {
  ...
  const handleAddGite = async (name, tasks, options = {}) => {
    ...
    if (Object.keys(options).length > 0) await updateGite(gite.id, options)
    if (tasks.length > 0) {
  ...
  return (
    <div className="app">
      ...
      {gites.length === 0 ? (
        ...
        tab==='dashboard'
          // ← gites passé en prop, useGites() n'est plus appelé dans Dashboard
          ? <main className="main"><ErrorBoundary name="Dashboard"><Dashboard gites={gites}/></ErrorBoundary></main>
          : activeGite && <GiteApp gite={activeGite} tab={tab}/>
      )}
